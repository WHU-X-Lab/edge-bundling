...
function setDefaultAssets(gl){
    ...
    gl.quadBuffer = createBuffer(gl, new Float32Array([0, 0, 1, 0, 0, 1, 0, 1, 1, 0, 1, 1]));
 ...
 * }
 * @return {gl}                core handle of webGL
 */
function getGL(window, opt) {
    var canvas = window.canvas = document.getElementById('canvas');
    if (canvas == null){
        console.log("Warning: please set your webGL-output canvas's Id as 'canvas'!");
        return;
    }
    var option = opt || {};
    ...
    var gl = window.gl = canvas.getContext('webgl', {preserveDrawingBuffer : preserveDrawingBuffer});
    gl.canvas = canvas;
    if (!gl) {
